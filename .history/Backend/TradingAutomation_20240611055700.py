...
def fetch_historical_data(symbol, interval, limit=200):
    ...
    now = int(time.time())
    since = now - limit * int(interval[:-1]) * 60
    response = session.query_kline(symbol=symbol, interval=interval, limit=limit, from_time=since)
...
def backtest_strategy(symbol, interval, trading_type):
    print("symbol: " + symbol, "interval: " + interval, "trading_type: " + trading_type)
    df = fetch_historical_data(symbol, interval, limit=288)  # 24 hours of data for 5 minute intervals (12 data points per hour * 24 hours)
    print("<df> ", df)
    df = calculate_indicators(df)
    print("<calculated_indicators> ", df)

...
def fetch_historical_data(symbol, interval):
    ...
    limit = 1 if interval == "1d" else 1440/(int(formatted_interval))
    now = int(time.time())
    since = (now - 86400) * 1000
    response = session.query_kline(symbol=symbol, interval=formatted_interval, limit=limit, start=since)

...
def fetch_historical_data(symbol, interval):
    ...
    formatted_interval = intervals[interval]
    limit = 1440/formatted_interval
    now = int(time.time())
...
def backtest_strategy(symbol, interval, trading_type):
    df = fetch_historical_data(symbol, interval)  # 24 hours of data for 5 minute intervals (12 data points per hour * 24 hours)

...
def fetch_historical_data(symbol, interval):
    ...
    limit = 1440/(int(interval)) if interval == "1d" else 1440/(int(formatted_interval))
    now = int(time.time())
    since = now - limit * int(interval) * 60 if interval == "1d" else now - limit * int(formatted_interval) * 60
